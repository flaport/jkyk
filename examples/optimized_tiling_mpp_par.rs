...
// Raw pointer wrapper for better performance
struct RawArray {
    ptr: *mut f32,
    len: usize,
}
...
fn main() -> Result<()> {
    ...
    for mut i in 0..(2 * Q / H) {
        i *= H;
        for mvm in 0..MV {
            // Use plain parallel iterator instead of par_bridge for better performance
            (0..oms).into_par_iter().for_each(|om| {
                let mut fast = vec![0_f32; (W * W32 * W32 * C * F) as usize];
                let mut c1: isize;
                let mut c2: isize;
                let mut f: isize;
                let mut g: isize;
                ...
                let mut j1: isize;
                let mut j2: isize;
                let om = om * W; // om: offset in x/m direction
                for mut on in 0..ons {
                    on *= W; // on: offset in y/n direction
                    for mut op in 0..ops {
                        op *= W; // op: offset in z/p direction
                        unsafe {
                            fill_fast(&mut fast, &eh, om, on, op, mvm);
                        }
                        for h in 0..H {
                            f = h % 2;
                            g = 1 - f;
                        ...
                        unsafe {
                            extract_fast(&eh, &fast, om, on, op, mvm);
                        }
                    }
                }
            })
    ...
    let byte_data: Vec<u8> = unsafe {
        eh.as_slice()[..(2 * M * N * P * 3) as usize]
            .iter()
